...
class UserTest < ActiveSupport::TestCase
  def setup
    @user = User.new(name: "Test User", email: "test@example.com", password: "123123", password_confirmation: "123123")
  end
  ...
  test "should follow and unfollow a user" do
    tony = users(:tony)
    ayaya = users(:ayaya)
    assert_not tony.following?(ayaya)
    tony.follow(ayaya)
    assert tony.following?(ayaya)
    assert ayaya.followers.include?(tony)
    tony.unfollow(ayaya)
    assert_not tony.following?(ayaya)
  end
end
